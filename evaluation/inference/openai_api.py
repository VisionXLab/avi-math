...
def generate_response(row, source_dir, target_dir, vqa_processed):
    ...
    if row['qtype'] == 'multiple-choice':
        user_prompt = f"Observe this image captured by a drone and answer the question by choosing the best option. Question: {row['question']} Choices: {row['choices']} Let's think step by step." 
    elif row['qtype'] == 'yes/no':
        user_prompt = f"Observe this image captured by a drone and answer the question. Question: {row['question']} Let's think step by step."
    else:
        user_prompt = f"Observe this image captured by a drone and answer the question. Question: {row['question']} Let's think step by step."
    if row['context'] != '':
...
if __name__ == "__main__":
    target_dir = f"/path/to/avi-math/gpt4o_extract_ans/"
