...
def generate_response(row, source_dir, target_dir, vqa_processed):
    ...
    if row['qtype'] == 'multiple-choice':
        user_prompt = f"Question: {row['question']} Choices: {row['choices']} Describe the image information relevant to the question and options. Do not answer the choice question directly." 
    elif row['qtype'] == 'yes/no':
        user_prompt = f"Question: {row['question']} Describe the image information relevant to the question. Do not answer the question directly."
    else:
        user_prompt = f"Question: {row['question']} Describe the image information relevant to the question. Do not answer the question directly."
    ...
    if row['qtype'] == 'multiple-choice':
        user_prompt = outputs + "\n" + f"Use the image captured by a drone and context to answer the question by choosing the best option. Question: {row['question']} Choices: {row['choices']}" 
    elif row['qtype'] == 'yes/no':
        user_prompt = outputs + "\n" + f"Use the image captured by a drone and context to answer the question. Question: {row['question']}"
    else:
        user_prompt = outputs + "\n" + f"Use the image captured by a drone and context to answer the question. Question: {row['question']}"
...
if __name__ == "__main__":
    target_dir = f"/path/to/avi-math/gpt4o-dcot_extract_ans/"
